<script setup lang="ts">
// import type { UserInfoExt } from './types';

import { computed } from 'vue';

import { preferences, usePreferences } from '@vben/preferences';

import { Card, Descriptions, DescriptionsItem, Tooltip } from 'ant-design-vue';

import { userUpdateAvatar } from '#/api/system/profile';
import { CropperAvatar } from '#/components/cropper';

const props = defineProps<{ profile?: UserInfoExt }>();
defineEmits<{
  // 头像上传完毕
  uploadFinish: [];
}>();
const institutionAppellation = import.meta.env.VITE_INSTITUTION_APPELLATION;
const avatar = computed(
  () => props.profile?.avatar || preferences.app.defaultAvatar,
);

const { isDark } = usePreferences();
const poetrySrc = computed(() => {
  const color = isDark.value ? 'white' : 'gray';
  return `https://v2.jinrishici.com/one.svg?font-size=12&color=${color}`;
});
function updateAvatar() {
  return (fileCallback: any) => {
    return userUpdateAvatar(fileCallback, props.profile as UserInfoExt);
  };
}
</script>

<template>
  <Card :loading="!profile" class="h-full lg:w-1/3">
    <div v-if="profile" class="flex flex-col items-center gap-[24px]">
      <div class="flex flex-col items-center gap-[20px]">
        <Tooltip title="点击上传头像">
          <CropperAvatar
            :show-btn="false"
            :upload-api="updateAvatar()"
            :value="avatar"
            width="120"
            @change="$emit('uploadFinish')"
          />
        </Tooltip>
        <div class="flex flex-col items-center gap-[8px]">
          <span class="text-foreground text-xl font-bold">
            <!-- {{ profile.name }} -->
            {{ profile.userRealName || profile.name }}
          </span>
          <!-- https://www.jinrishici.com/doc/#image -->
          <img :src="poetrySrc" />
        </div>
      </div>
      <div class="px-[24px]">
        <Descriptions :column="1">
          <DescriptionsItem label="用户ID">
            {{ profile.id }}
          </DescriptionsItem>
          <DescriptionsItem label="账号">
            {{ profile.username }}
          </DescriptionsItem>
          <DescriptionsItem label="用户类型">
            <!-- <Tag color="processing"> -->
            {{ profile.userType }}
            <!-- </Tag> -->
          </DescriptionsItem>
          <DescriptionsItem label="手机号码">
            {{ profile.mobile }}
          </DescriptionsItem>
          <!-- <DescriptionsItem label="邮箱">
            {{ profile.email || '未绑定邮箱' }}
          </DescriptionsItem> -->
          <DescriptionsItem :label="institutionAppellation">
            <!-- <Tag color="processing"> -->
            {{ profile.orgName }}
            <!-- </Tag> -->
            <!-- <Tag v-if="profile.postGroup" color="processing">
              {{ profile.postGroup }}
            </Tag> -->
          </DescriptionsItem>
          <!-- <DescriptionsItem label="上次登录">
            {{ profile.loginDate }}
          </DescriptionsItem> -->
        </Descriptions>
      </div>
    </div>
  </Card>
</template>
