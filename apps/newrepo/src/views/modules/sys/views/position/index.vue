...
const [Grid, gridApi] = useCommonGrid(
  {
    ...
    gridEvents: {
      radioChange({ row }: { row: any }) {
        parentTableParams.positionId = row?.id;
        subGridApi.query();
      },
    },
  },
  {
    dataTableId: '/datatable/data/page/sys.position',
  },
);
...
const [SubGrid, subGridApi] = useCommonGrid(
  ...
  {
    dataTableId: '/datatable/data/page/sys.positionRole',
    parentTableParams,
  },
);
        ...
        <SubGrid>
          <template #toolbar-actions>
            <Button
              type="primary"
              @click="handleSubAdd()"
              :disabled="!parentTableParams.positionId"
            >
              新增
              <template #icon>
                <AntdPlusCircleTwotone class="mb-[2px] text-[16px]" />
