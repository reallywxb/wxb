...
import inoutDetailModalUI from '../dailyEnd/modals/inoutDetailModal.vue';
...
const [ChcGrid, ChcGridApi, { handleExport }] = useSpdGrid(
  ...
  },
  {
    ...
    gridColumns: [
      ...
      { field: 'guaranteeDate', title: '有效期', width: '100', sortable: true },
      {
        field: 'beginQty',
        title: '期初数量',
        align: 'right',
    ...
    formSchema: [
      ...
      },
      {
        ...
        dependencies: {
          ...
          trigger(values: any) {
            nextTick(() => {
              const ref = ChcGridApi.formApi?.getFieldComponentRef('warehouseId') as any;
              if (ref?.params) {
      ...
      <ChcGrid class="flex-1 overflow-hidden">
        <template #toolbar-actions>
          <Button
            ...
            @click="handleExport"
            class="mr-[0.5rem]"
            data-testid="button_export_dailyEndLotQuery"
          >
            导 出
            <template #icon>
              <ExportActionIcon />
        ...
        <template #inStockQty="scope">
          <a
            ...
            :data-testid="`button_inStockQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
        ...
        <template #POQty="scope">
          <a
            ...
            :data-testid="`button_POQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.POQty }}
          </a>
        </template>
        <template #MIQty="scope">
          <a
            ...
            :data-testid="`button_MIQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.MIQty }}
          </a>
        </template>
        <template #SRQty="scope">
          <a
            ...
            :data-testid="`button_SRQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.SRQty }}
          </a>
        </template>
        <template #PSRQty="scope">
          <a
            ...
            :data-testid="`button_PSRQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.PSRQty }}
          </a>
        </template>
        <template #IIQty="scope">
          <a
            ...
            :data-testid="`button_IIQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.IIQty }}
          </a>
        </template>
        <template #outStockQty="scope">
          <a
            ...
            :data-testid="`button_outStockQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
        ...
        <template #PRQty="scope">
          <a
            ...
            :data-testid="`button_PRQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.PRQty }}
          </a>
        </template>
        <template #MOQty="scope">
          <a
            ...
            :data-testid="`button_MOQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.MOQty }}
          </a>
        </template>
        <template #SOQty="scope">
          <a
            ...
            :data-testid="`button_SOQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.SOQty }}
          </a>
        </template>
        <template #PSOQty="scope">
          <a
            ...
            :data-testid="`button_PSOQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.PSOQty }}
          </a>
        </template>
        <template #IOQty="scope">
          <a
            ...
            :data-testid="`button_IOQty_${scope.rowIndex}_dailyEndLotQuery`"
          >
            {{ scope.row.IOQty }}
          </a>
        </template>
...
::v-deep(.vxe-table-export--panel-column .vxe-table-export--panel-column-body) {
  /* 隐藏第一个li */
  li:first-child {
    display: none;
  }
