...
const handleSave = async () => {
  ...
  params.asnId = modalOuterData.value.asnId;
  Modal.confirm({
    title: '提示',
    content: '确认入库？',
    okText: '确认',
    ...
    onOk() {
      requestFormClient
        .post('/asnAction/rejectReceivePackage.do', params)
        .then(() => {
          modalOuterData.value.reloadParentTable();
          // chcGridApi.formApi.setFieldValue('scanedPackageCount', 0);
          scanModalApi.close();
          ...
          <div class="flex items-center justify-center pt-[10px]">
            <div class="flex gap-[10px]">
              <Button
                type="primary"
                @click="handleSave"
                data-testid="button_save_scanModal"
              >
                确认入库
                <template #icon>
                  <SvgSaveIcon />
