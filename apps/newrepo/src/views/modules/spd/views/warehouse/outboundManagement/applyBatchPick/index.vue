  ...
  },
  {
    ...
    beforeFetchFn: (params: any) => {
      return {
        ...
        orderIds: params?.orderIds || undefined,
...
const [ChcGrid, chcGridApi, { EditModal, editModalApi }] = useSpdGrid(
  {
    formOptions: {
      ...
      handleSubmit: async () => {
        ...
        chcGridApi.formApi.setLatestSubmissionValues(toRaw(formValues));
        chcGridApi.query(formValues);
        // 父表查询后，同步更新子表（只带单选的orderId）
        // if (parentTableParams.value.orderId) {
        //   childGridApi.reload({ orderId: parentTableParams.value.orderId });
        // } else {
  ...
  },
  {
    ...
    gridEvents: {
      checkboxChange: ({ records }: { records: any[] }) => {
        handlePrescriptionChecked(records);
      },
    ...
    afterFetchFn: (params) => {
      ...
      try {
        ...
        setTimeout(() => {
          ...
          handlePrescriptionChecked(rows);
        });
        return { records: rows, total: params.total } as any;
      } catch (error) {
        console.error('afterFetchFn parse error', error);
...
const handleSearch = () => {
  ...
  if (selectedPrescriptionIds.value.length === 0) {
    return message.warn('请先勾选单据！');
  }
  const orderIds = selectedPrescriptionIds.value.join(',');
  childGridApi.reload({
 ...
 * @param row 当前行数据
 */
const handleQtyProcessBlur = (row: any) => {
  console.warn('指示数量失焦-当前行数据:', JSON.parse(JSON.stringify(row)));

  // AI-GENERATED-BEGIN
  // @date 2026-06-17
  // @prompt 指示数量失焦后保存数量到后端
  // @description 调用接口保存当前行的指示数量，保存成功后重新查询子表
  requestFormClient
    .post('orderAction/saveLineQtyProcess.do', {
      orderLineId: row.orderLineId,
      qtyProcess: row.qtyProcess,
    })
    .then((res) => {
      console.warn('保存指示数量成功:', res);
      // 保存成功后重新查询子表
      if (res && res.success) {
        const orderIds = selectedPrescriptionIds.value.join(',');
        childGridApi.query({
...
const handleCreatePoPlan = (isOnly: string, isCrossDocking: string) => {
  ...
  lineRecords.forEach((record: any, index: number) => {
    // 1. 必须有 productId
    // 2. qtyPo（缺货数量）> 0
    // 3. qtyPo <= qtyLeft（待发数量）
    // 4. warehouseId 一致
    // 5. 直配采购时订单类型必须是 WO 或 SO
    // console.warn('lineRecords:', record, params.warehouseId);
    if (!record.productId) {
  ...
  Modal.confirm({
    ...
    cancelText: '取消',
    onOk() {
      console.warn('转直配采购/转采购params:', params);
      //debugger;
      requestFormClient
        .post('orderPlanAction/creatOrderPlanByApplyBatch.do', params)
        .then(() => {
          chcGridApi.query();
        });
    },
...
const handleCreate = () => {
  ...
  console.warn('指示-params:', params);
  //debugger;
  requestFormClient
    .post('orderAction/preCheckOutLimitCheckBatch.do?page=picklist', params)
    .then((res) => {
      if (res.isTips) {
        Modal.confirm({
          title: '提示',
...
const outputApprove = () => {
  ...
  // AI-GENERATED-BEGIN
  // @date 2026-06-16
  // @prompt 将lineData按orderId分组，生成多订单格式
  // @description 将平铺的lineData按orderId分组，生成[{orderId, lineData:[]}]格式，支持多订单指示
  ...
  lineData.forEach((item) => {
    ...
    groupedData[oid].push({
      orderLineId: item.orderLineId,
      qtyProcess: item.qtyProcess,
      orderId: item.orderId,
    });
  });
  const resultParams = Object.keys(groupedData).map((oid) => ({
    orderId: Number(oid),
    lineData: JSON.stringify(groupedData[oid]),
  }));
  // AI-GENERATED-END
  console.warn('指示二次-params:', resultParams);
  //debugger;
  Modal.confirm({
    title: '提示',
    content:
    ...
    onOk() {
      ...
      requestFormClient
        .post('orderAction/outputApproveBatch.do', {
          batchData: JSON.stringify(resultParams),
        })
        .then((res) => {
          if (res.data && res.data.needPrint === 'Y') {
            ...
            Modal.confirm({
              ...
              onOk() {
                // AI-GENERATED-BEGIN
                // @date 2026-06-16
                // @prompt 批量打印拣货单，每个拣货单号单独打印
                // @description 兼容res.data.ids是数组或字符串，拆分后每个单号生成一个打印任务，调用batchPrint批量打印
                // const idsStr = Array.isArray(res.data.ids)
                //   ? res.data.ids.join(',')
                //   : res.data.ids;
                // const ids = idsStr.split(/[,，]/); // 支持中英文逗号分割
                // const tasks = ids.map((id: string) => ({
                //   pdf_path: `${location.origin}${import.meta.env.VITE_GLOB_API_URL}/pickListAction/printPickListDoc.do?id=${id.trim()}`,
                // }));
                ...
                globalPrintStore.print({
                  pdf_path: `${location.origin}${import.meta.env.VITE_GLOB_API_URL}/pickListAction/printPickListDoc.do?id=${res.data.ids}`,
                });
                // AI-GENERATED-END
              },
              onCancel() {},
            ...
            // 未打印就跳出了拣货单号
            message.success(`${res.data.msg}`, 8000);
...
// 点击关闭
// 行操作：点击关闭
const handleRowClose = (row: any) => {
  Modal.confirm({
    title: '提示',
    content: `确认关闭申请单 ${row.orderNo} 吗？`,
    okText: '确认',
    cancelText: '取消',
    onOk() {
      requestFormClient
        .post('orderAction/close.do', {
          orderId: JSON.stringify([row.orderId]),
        })
        .then(() => {
...
const selectedPrescriptionIds = ref<Array<number | string>>([]); // 选中的父表ID

const handlePrescriptionChecked = (records: any[]) => {
  // 更新选中的处方ID
  selectedPrescriptionIds.value = records.map((item) => item['orderId']);
  const selectedPrescriptionStr = selectedPrescriptionIds.value.join(',');
  childTotal.value = 0;
  // 只有当有选中记录时才查询子表
  if (records.length > 0) {
    // 查询处方下的药品
    childGridApi.query({
      orderIds: selectedPrescriptionStr, // "123,456,789"
    });
  } else {
    // 清空子表
    childGridApi.grid.remove(childGridApi.grid.getFullData());
          ...
          <ChildGrid>
            <template #qtyProcessDefault="scope">
              <InputNumber
                ...
                @blur="handleQtyProcessBlur(scope.row)"
              />
            </template>
