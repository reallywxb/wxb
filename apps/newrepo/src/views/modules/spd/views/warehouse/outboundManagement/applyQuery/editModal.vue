...
const [Modal, modalApi] = useVbenModal({
  ...
  onOpenChange(isOpen: boolean) {
    if (isOpen) {
      modalOuterData.value = modalApi.getData<{
        handleRefreshTable: () => void;
        lineData: any;
        tableId: string;
      }>();
...
const chcGridOption: SchemaColumnAndOptions = {
  ...
  queryUrl: () =>
    `/orderAction/queryLine.do?specShowType=from&page=applyPick&orderId=${orderId.value}`,
...
const handleSave = async () => {
  ...
  // console.log('handleSave:', params, records);
  // modalApi.close();
  const created: any[] = [];
  const updated: any[] = [];
  let total = 0;
      ...
      <ChcGrid>
        <template #descriptionDefault="scope">
          <Input
            ...
            :data-testid="`input_description_${scope.rowIndex}_applyQuery`"
          />
        </template>
        <template #qtyOrderedDefault="scope">
          <InputNumber
            ...
            :data-testid="`input_qtyOrdered_${scope.rowIndex}_applyQuery`"
          />
        </template>
        <template #toolbar-actions>
          <Button
            ...
            danger
            @click="handleDel"
            data-testid="button_delete_applyQuery"
          >
            删除
            <template #icon>
              <SvgDeleteIcon />
        ...
        <template #toolbar-tools>
          <Input
            ...
            data-testid="input_serachInputVal_applyQuery"
          />
          <Button
            type="primary"
            @click="handleSearch"
            data-testid="button_search_applyQuery"
          >
            搜索
            <template #icon>
              <SearchActionIcon />
          ...
          <div class="flex items-center justify-center pt-[10px]">
            <div class="flex gap-[10px]">
              <Button
                type="primary"
                @click="handleSave"
                data-testid="button_save_applyQuery"
              >
                保存
                <template #icon>
                  <SvgSaveIcon />
              ...
              <Button
                ...
                data-testid="button_cancel_applyQuery"
              >
                取消
                <template #icon>
                  <SvgCloseIcon />
