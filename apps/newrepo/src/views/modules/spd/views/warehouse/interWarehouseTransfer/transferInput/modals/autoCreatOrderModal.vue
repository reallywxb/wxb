...
const [Modal, modalApi] = useVbenModal({
  ...
  onOpenChange(isOpen) {
    if (isOpen) {
      queryData.value = modalApi.getData<Record<string, any>>();
      setTimeout(() => {
        chcGridApi.formApi.setValues(queryData.value);
      }, 200);
    } else {
      handleReset();
    }
...
const total = ref(0);
const EDITABLE_FIELDS = new Set(['qtyPlan', 'qtyPlanPackage']);
const [ChcGridUI, chcGridApi] = useSpdGrid(
  ...
  },
  {
    ...
    formSchema: [
      {
        ...
        componentProps: () => {
          return {
            ...
            showSearch: true,
            disabled: true,
            defaultValue: queryData.value.departmentId,
            placeholder: '请选择院区',
            paginate: false,
            // allowClear: true,
      ...
      },
      {
        ...
        componentProps: () => {
          return {
            ...
            paginate: false,
            allowClear: true,
            defaultValue: queryData.value.warehouseId,
            disabled: true,
            // onChange(val: any, option: any) {
      ...
      },
      {
        ...
        componentProps: () => {
          return {
            ...
            placeholder: '请选择调入仓库',
            disabled: true,
            defaultValue: queryData.value.toWarehouseId,
            onChange(val: any, option: any) {
    ...
    beforeFetchFn: (params) => {
      return {
        ...params,
        toWarehouseId: queryData.value.toWarehouseId,
        warehouseId: queryData.value.warehouseId,
        isPackaged: 'N',
        start: undefined,
        limit: 0,
      };
...
const getNoetMessage = () => {
  chcGridApi.formApi.getValues().then((resData: any) => {
    ...
    let msg = '补货数量：【流水数量';
    if (resData.subStorage === 'Y') {
      msg = `${msg}-当前库存数量`;
    }
    if (resData.subOrdering === 'Y') {
      msg = `${msg}-在途数量`;
    }
...
const handleCreated = () => {
  ...
  const noQtyPlanDataIndex = selectedRows.findIndex((row: any) => !row.qtyPlan);
  if (noQtyPlanDataIndex !== -1) {
    message.warning(
      `选中的第${noQtyPlanDataIndex + 1}行数据，请先填写请领数量`,
    );
    return;
  }
  // 有选中数据才弹出确认框
