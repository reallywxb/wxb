...
import { invalidate } from '#/views/modules/spd/views/fin/settlement/api';
      ...
      gridOptions: {
        columns: [
          ...
          },
          {
            field: 'origPricePo',
            title: '采购入库价',
            formatter: ({ cellValue }: any) => handlePriceToFixedTwo(cellValue),
            width: '100',
            align: 'right',
            sortable: true,
          },
...
function invalidateLine(row: any) {
  Modal.confirm({
    title: '提示',
    content: '确认作废结算行吗？',
    okType: 'danger',
        ...
        <ChildGrid>
          ...
          <template #action="scope">
            <Button
              ...
              :data-testid="`button_invoiceDetail_${scope.rowIndex}`"
            >
              发票明细
              <template #icon>
                <IconfontBasicView />
            ...
            <Button
              ...
              type="primary"
              class="mr-[6px] h-[24px] pb-0 pl-[6px] pr-[6px] pt-0"
              @click="invalidateLine(scope.row)"
              :data-testid="`button_invalidateLine_${scope.rowIndex}`"
            >
              作废
              <template #icon>
                <IconfontBasicView />
