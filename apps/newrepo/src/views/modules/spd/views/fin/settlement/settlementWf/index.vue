...
import { rejectWorkflow } from '#/views/modules/spd/views/fin/invoice/api';
import {
  deleteLine,
  modifyPrice,
...
import { approveWorkflow } from '#/views/modules/spd/views/warehouse/warehouseOrder/orderReturnApproval/api';
...
const [ParentGrid, parentGridApi] = useSpdGrid(
  ...
  },
  {
    ...
    afterFetchFn: (params) => {
      console.warn('afterFetchFn res', params);
...
// 父表 - 拒绝对话框
const [SettlementRejectModal, settlementRejectModalApi] = useVbenModal({
  class: 'w-[400px]',
  closable: true,
  // 连接抽离的组件
...
function handleReject() {
  ...
  settlementRejectModalApi
    .setData({
      title: `拒绝${selectedRows.length}笔申请`,
      submit: (params: any) =>
        rejectWorkflow({
          ...params,
          wfActivityId: JSON.stringify(
    ...
    >
      <template #first>
        ...
        <SettlementRejectModal
          :form-options="rejectFormOptions"
        ...
        <ParentGrid>
          ...
          <template #action="scope">
            <Button
              ...
              :data-testid="`button_approvalProcess_${scope.rowIndex}`"
            >
              查看审批流程
              <template #icon>
                <IconfontBasicView />
