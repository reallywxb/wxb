...
const getGridConfig: (conf: any) => [GridConfig, any] = (pageConfig: any) => {
  ...
  const getOptions = (items: any[]) => {
    ...
    const newItems: any[] = [];
    items.forEach((item) => {
      newItems.push({ value: item.value, label: item.name });
    });
    return newItems;
  };
  ...
  const toolbarConfig = {
    actions: {
      export: getAttrValue(pageConfig.toolbar.actions, 'export') === 'true',
    },
  };
...
onMounted(async () => {
  const response = await getPageConfig({ menuId: urlParams.menuId });

  // 根据页面定义生成Grid配置
  const [gridConfig, toolbarConf] = getGridConfig(response.data);
  toolbarConfig = toolbarConf;
  const [ChcGrid1, ChcGridApi, { handleExport }] = useSpdGrid(
    {
      formOptions: {
    ...
    },
    {
      ...
      dataTableId: `/baseHandleAction/invokeEngin.do?menuId=${urlParams.menuId}`,
      showCustomBtn: true,
      showZoomBtn: true,
      showExportBtn: true,
      id: 'crud',
