  ...
  async onConfirm() {
    ...
    if (validateResult.valid) {
      ...
      data.value.records.forEach((item: any) => {
        item.toLocatorId = formValues.locatorIdTo;
        ...
        item.toLocatorName = nameObj.value.toLocatorName;
      });
      modalApi.close();
    }
  ...
  schema: [
      ...
      componentProps: () => {
        return {
          ...
          onChange(_: any, option: any) {
            nameObj.value.toLocatorName = option?.name;
          },
        };
      },
      rules: 'required',
    ...
    },
    {
      ...
      componentProps: () => {
        return {
          dictUrl:
            "/baseHandleAction/refList.do?id=1000346&validation=ad_ref_list.value in('H','S','R')",
          placeholder: '请选择',
