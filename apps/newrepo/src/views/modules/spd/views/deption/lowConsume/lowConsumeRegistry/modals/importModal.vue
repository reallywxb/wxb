...
import excelUrl from '#/assets/excels/packageDetail.xls?url';
import shipmentPackage from '#/assets/excels/shipmentPackage.base64?raw';

import { importPoPlanData } from '../api';
...
const handleChange = (info: UploadChangeParam<any>) => {
  baseFormApi.getValues().then((res) => {
    ...
    // // 关闭模态框
    // modalApi.close();
    importPoPlanData({ file: info.file }).then((res) => {
      if (res.success) {
...
<template>
  <Modal class="w-[500px]" title="导入" title-tooltip="上传excel文件导入数据">
    ...
    <div class="pl-[10px] pr-[10px]">
      ...
      >
        <AntdDownloadOutlined class="mr-[6px] text-[14px]" />包装明细模版下载
      </a>
      <UploadDragger
        ...
        class="mt-[16px]"
        data-testid="button_upload__importModal"
      >
