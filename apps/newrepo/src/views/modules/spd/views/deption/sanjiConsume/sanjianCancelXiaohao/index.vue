...
const warehouseLevel =
  urlParams?.warehouseLevel ||
  (route.name ===
  'spd.web.wms.w3.warehouse.order.inoutOutput.revert.sanjianCancelXiaohao'
    ? '2'
    : '3');
const isWarehouseConsumptionReversed =
  route.name ===
  'spd.web.wms.w3.warehouse.order.inoutOutput.revert.sanjianCancelXiaohao';
...
const handleFormReset = async () => {
  ...
  // 重置后重新触发院区字段的数据获取，确保自动选择第一个选项
  // if (
  //   chcGridApi.formApi?.getFieldComponentRef &&
  //   typeof chcGridApi.formApi?.getFieldComponentRef === 'function' &&
  //   chcGridApi.formApi?.getFieldComponentRef('departmentId')
  // ) {
  //   await chcGridApi.formApi?.getFieldComponentRef('departmentId')?.fetchApi();
  // }
  ...
  // console.log(formValues?.warehouseId, 'formValues?.warehouseId');
...
const [ChcGrid, chcGridApi, { BatchSetModal, batchSetModalApi }] = useSpdGrid(
  ...
  },
  {
      ...
      },
      {
        ...
        componentProps: () => {
          return {
            autoChooseFirstOption: true,
            // dictUrl:
            //   '/baseHandleAction/warehouse.do?level3=Y&readWrite=Y&isProxyWarehouse=N',
            dictUrl: `/baseHandleAction/warehouse.do?level${warehouseLevel}=Y${isWarehouseConsumptionReversed ? '&level3=Y' : ''}&readWrite=Y&isProxyWarehouse=N`,
            // showSearch: true,
            placeholder: '请选择仓库',
    ...
    queryUrl: 'returnApplyAction/queryV3InOutDetail.do?page=V3RevertConsume',
    showCustomBtn: true,
    showZoomBtn: true,
    id: 'sanjianCancelXiaohao',
...
const handleBatchSet = async () => {
  ...
  records.forEach((data: any) => {
    ...
    if (!data.qtyOrdered && data.qtyOrdered !== 0) {
      hasError = true;
      return message.error(`药品：${data.productName}，撤销数量不存在`);
    }
    // 检查撤销数量是否大于可用数量
    if (data.qtyCanReturn < data.qtyOrdered) {
      hasError = true;
      return message.error(
        `药品：${data.productName}，撤销数量：${
          data.qtyCanReturn
    ...
    if (data.qtyOrdered <= 0) {
      hasError = true;
      return message.error(`药品：${data.productName}，撤销数量不可小于0`);
    }
  });
