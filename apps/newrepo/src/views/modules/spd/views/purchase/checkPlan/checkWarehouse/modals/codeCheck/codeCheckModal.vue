...
const [Modal, modalApi] = useVbenModal({
  ...
  async onConfirm() {
    //  modalApi.close(); // 禅道1356 提交成功后关闭弹窗
...
//   let qty = 0;
//   const currentData = ChcGridApi.grid.getTableData().tableData || [];
//   const packageNos = currentData.map((row) => {
//     qty += row.qty || 0;
...
//     return;
//   }
//   if (modalOuterData.value.qtyArrived > qty) {
//     AntModal.confirm({
//       title: '提示',
//       content: `当前还存在包装未扫描，提交后未扫包装将正常入库，是否确认提交？`,
//       onOk: async () => {
//         modalRejCodeApi
//           .setData({
...
//   }
// };
const [OrgFormModal] = useVbenModal({
  class: 'w-[500px]',
  closable: true,
  // 连接抽离的组件
