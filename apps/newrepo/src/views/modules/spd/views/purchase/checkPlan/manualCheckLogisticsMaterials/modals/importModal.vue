...
import { importAsnSuppliesData } from '#/api/system/import';
import asn from '#/assets/excels/asn.base64?raw';
import excelUrl from '#/assets/excels/asnSupplies.xls?url';
...
const handleChange = (info: UploadChangeParam<any>) => {
  baseFormApi.getValues().then((res) => {
    importAsnSuppliesData({ file: info.file, ...res, ...extParams.value }).then(
      (res) => {
        if (res.success) {
