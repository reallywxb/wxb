...
const [ChcGrid, chcGridApi, { EditModal }] = useSpdGrid(
  {
    ...
    gridOptions: {
      ...
      rowStyle(scope: any) {
        if (scope.row.processStatus === 'C') {
          return {
            color: 'red',
          };
        }
        if (scope.row.processStatus === 'Y') {
          return {
            color: 'green',
          };
        }
  ...
  {
    id: 'parent',
    queryUrl: `orderAction/query.do?isSurgery=Y`,
    gridColumns: [
      { title: '单选', type: 'radio', visible: false },
      {
        title: '序号',
      ...
      // 	   if(item.processStatus == 'C'){//关闭
      // 	      return "red";
      // 	   }
      // 	   if(item.processStatus == 'Y'){//关闭
      // 		      return "green";
      // 	   }
      // 	}
      // },
      ...
      },
      {
        ...
        sortable: true,
        formatter({ row }: any) {
          // 只有无审批流程且处理状态是WU的才能显示为“待消耗”
          if (row.processStatus === 'WC' && row.docStatus !== 'WA') {
            return '待消耗';
          }
          return row.processStatusName;
        },
      },
      {
      ...
      },
      {
        ...
        componentProps: () => {
          return {
            // autoChooseFirstOption: true,
            dictUrl:
              '/baseHandleAction/warehouseBPartner.do?readWrite=Y&level1=N&selectDefault=Y',
            placeholder: '请选择执行科室',
            paginate: false,
            showChooseAll: '',
            immediate: true,
...
const handlePrintHbtn = () => {
  console.warn('药品-打印跟台单标签');
  ...
  if (!isDocOK) {
    return message.error('仅已确认的跟台单可以打印标签');
  }
  Modal.confirm({
    title: '打印提示',
    content: '确认打印标签吗？',
    ...
    onOk() {
      globalPrintStore.print({
        pdf_path: `${location.origin}${import.meta.env.VITE_GLOB_API_URL}/packageAction/printPackageDocByOrder.do?ids=${encodeURIComponent(JSON.stringify(paramLine))}`,
      });
    },
    onCancel() {},
  });
};
const handlePrintDbtn = () => {
  const record = chcGridApi.grid.getRadioRecord(true);
  if (!record) {
    return message.error('请选择跟台单');
  }
  if (record.docStatus !== 'CO') {
    return message.error('仅已确认的跟台单可以打印标签');
  }
  const item = childGridApi.grid.getRadioRecord(true);

  const paramLine = [item.orderLineId];
  Modal.confirm({
    title: '打印提示',
    content: '确认打印标签吗？',
    okText: '确认',
    ...
    onOk() {
      globalPrintStore.print({
        pdf_path: `${location.origin}${import.meta.env.VITE_GLOB_API_URL}/packageAction/printPackageDocByOrder.do?lineIds=${encodeURIComponent(JSON.stringify(paramLine))}`,
      });
    },
    onCancel() {},
  });
          ...
          <ChcGrid>
            <template #toolbar-actions>
              <Button
                type="primary"
                class="mr-[0.5rem]"
                @click="handlePrintHbtn"
                data-testid="button_handlePrintHbtn"
              >
                打印标签
                <template #icon>
                  <SvgPrintFillIcon />
        ...
        <template #second>
          <ChildGrid>
            ...
            <template #toolbar-actions>
              ...
              <Button
                type="primary"
                @click="handlePrintDbtn"
                class="mr-[0.5rem]"
                data-testid="button_handleChildPrint"
              >
                打印标签
                <template #icon>
                  <SvgPrintFillIcon />
