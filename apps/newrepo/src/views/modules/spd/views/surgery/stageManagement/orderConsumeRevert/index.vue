...
import { SearchActionIcon, SvgBackIcon } from '@vben/chc-icons';
...
const [ChildGrid, childGridApi] = useSpdGrid(
  ...
  },
  {
    ...
    id: 'child',
    queryUrl:
      'surgeryAction/queryOrderLinePackage.do?page=surgeryPackageRevert',
...
const handleRevert = () => {
  ...
  if (record.docStatus !== 'CO') {
    return message.error('仅已确认的跟台单可以撤销消耗');
  }
  ...
  records.forEach((data: any, i: number) => {
    if (data.packageStatus !== 'O') {
      message.error(`第${i + 1}行: 包装在库！`);
      error = true;
      return;
    }
    paramLine.push(data.packageId);
  });
  if (error) {
    return null;
  }
  Modal.confirm({
    ...
    onOk() {
      ...
      param.lineIds = JSON.stringify(paramLine);
      requestFormClient
        .post('surgeryAction/surgeryConsumeRevertByPackage.do', param)
        .then(() => {
          handleSearch();
        });
    },
          ...
          <ChildGrid>
            <template #toolbar-actions>
              ...
              <Button
                ...
                class="mr-[0.5rem]"
                @click="handleRevert"
                data-testid="button_handleRevert"
              >
                撤销消耗
                <template #icon>
                  <SvgBackIcon />
