...
const [ChcGrid, chcGridApi, { RejectModal, rejectModalApi }] = useSpdGrid(
  ...
  },
  {
    id: 'parent',
    queryUrl:
      '/orderAction/query.do?page=surgeryApproveConsume&isSurgery=Y&orderType=PO&docStatus=%27WU%27,%27WA%27',
    gridColumns: [
...
const handlePass = () => {
  ...
  Modal.confirm({
    ...
    cancelText: '取消',
    onOk() {
      const remove = [record.orderId];
      const param: { [key: string]: any } = {};
      param.orderId = JSON.stringify(remove);
      param.isSkipApproved = urlParams.isSkipApproved || '';
      requestFormClient.post(`orderAction/commitSurgery.do`, param).then(() => {
