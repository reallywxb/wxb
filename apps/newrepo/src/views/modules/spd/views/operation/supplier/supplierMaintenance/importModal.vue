...
const url = new URL('#/assets/excels/vendor.xls', import.meta.url).href;
...
const handleChange = (info: UploadChangeParam<any>) => {
  baseFormApi.getValues().then((res) => {
    importVendor({ file: info.file, ...res, ...extParams.value }).then(
      (res) => {
        if (res.success) {
  ...
  <Modal class="w-[500px]" title="导入" title-tooltip="上传excel文件导入数据">
    ...
    <div class="pl-[10px] pr-[10px]">
      <a
        ...
        data-testid="a_download_importModal"
      >
        <AntdDownloadOutlined class="mr-[6px] text-[14px]" />供应商模版下载
      </a>
      <UploadDragger
