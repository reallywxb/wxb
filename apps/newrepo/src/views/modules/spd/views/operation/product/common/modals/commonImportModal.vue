...
async function handleChange(info: UploadChangeParam<any>) {
  ...
  try {
    const values = props.formSchemas?.length ? await formApi.getValues() : {};
    const filteredValues = filterNullUndefined(values);
    const { success, msg, data } = await props.httpRequest({
      file: info.file,
      ...filteredValues,
    ...
    <div class="pl-[10px] pr-[10px]">
      ...
      <template v-if="templateUrl">
        <a
          ...
          data-testid="a_downloadTemplate_commonImportModal"
        >
      ...
      <UploadDragger
        ...
        class="mt-[16px]"
        data-testid="upload_UploadDragger_commonImportModal"
      >
