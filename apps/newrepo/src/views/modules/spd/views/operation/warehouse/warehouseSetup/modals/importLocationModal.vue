...
import excelUrl from '#/assets/excels/locator.xls?url';
import shipmentPackage from '#/assets/excels/shipmentPackage.base64?raw';

import { importLocatorData } from '../api';
...
const handleChange = (info: UploadChangeParam<any>) => {
  baseFormApi.getValues().then((res) => {
    ...
    // 弹窗加锁，显示loading
    modalApi.lock();
    importLocatorData({ file: info.file })
      .then((res) => {
        modalApi.unlock();
        if (res.success) {
  ...
  >
    ...
    <div class="pl-[10px] pr-[10px]">
      <a
        ...
        data-testid="button_download_importLocationModal"
      >
        <AntdDownloadOutlined class="mr-[6px] text-[14px]" />仓库货位模板下载
      </a>
      <UploadDragger
        ...
        class="mt-[16px]"
        data-testid="button_UploadDragger_importLocationModal"
      >
