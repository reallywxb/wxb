...
import supplierDetail from '../modal/supplierDetail.vue';
...
const handleReject = () => {
  ...
  Modal.confirm({
    ...
    content: h('textarea', {
      ...
      'data-testid': 'textarea_reject_reason_supplierLicense',
    }),
    onOk: async () => {
      const rejectReason = (
...
const [SupplierDetailModal, SupplierDetailModalApi] = useVbenModal({
  class: 'w-[600px]',
  closable: true,
  // 连接抽离的组件
  connectedComponent: supplierDetail,
  draggable: true,
});
const handleLicenseDetail = (row: any) => {
  console.warn('row', row);
  SupplierDetailModalApi?.setData({
    row,
    callback() {
      // 刷新表格数据
      ChcGridApi.query({
    ...
    <ChcGrid>
      <template #toolbar-actions>
        ...
        <Input
          ...
          allow-clear
          data-testid="input_certNo_supplierLicense"
        />
        <Button
          type="primary"
          @click="handleSearch"
          class="mr-[0.5rem]"
          data-testid="button_search_supplierLicense"
        >
          查询
          <template #icon>
            <SearchActionIcon />
        ...
        <Button
          ...
          class="mr-[0.5rem]"
          data-testid="button_approval_supplierLicense"
        >
          通过
          <template #icon>
            <SvgSquareTickIcon />
        ...
        <Button
          ...
          @click="handleReject"
          class="mr-[0.5rem]"
          data-testid="button_reject_supplierLicense"
        >
          拒绝
          <template #icon>
            <SvgCloseIcon />
      ...
      <template #action="scope">
        <Button
          ...
          :data-testid="`button_view_license_${scope.rowIndex}_supplierLicense`"
        >
          证照详情
          <template #icon>
            <IconfontBasicView />
