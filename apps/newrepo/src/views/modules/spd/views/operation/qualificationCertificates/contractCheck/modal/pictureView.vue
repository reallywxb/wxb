  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        ...
        const params = {
          checkRemark: '',
          type: 'Contract', //  合同证照类型固定为 Contract
          status: isWA ? 'WC' : 'PS', // 根据预审状态决定目标状态
          ids: JSON.stringify([
            {
              applySyncId: state.value.applyId, // 外层申请ID
              applyCertId: data.contracteApplyId, // 合同证照ID
            },
          ]),
        };

        console.warn('合同证照通过操作参数:', params);
...
const handleReject = () => {
  ...
  Modal.confirm({
    ...
    content: h('textarea', {
      ...
      'data-testid': 'textarea_reject_reason_pictureView',
    }),
    width: '400px',
    centered: true,
    async onOk() {
      ...
      try {
        ...
        const params = {
          ...
          type: 'Contract', // 合同证照固定为 Contract
          status: 'NO', // 拒绝状态固定为NO
          ids: JSON.stringify([
            {
        ...
        console.warn('合同证照拒绝操作参数:', params);
        const result = await passLicenseViewPicture(params);
        ...
        <div class="flex h-full flex-col gap-2 px-2.5 py-3">
          <div class="flex h-20 w-full border border-[#b1b1b1] py-2">
            ...
            <div
              ...
              :data-testid="`button_view_license_${imageIndex}_pictureView`"
            >
              <div
