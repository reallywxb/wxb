...
import productorgUrl from '#/assets/excels/productorg.xls?url';

import { importProductOrg } from './api';
...
const handleChange = (info: UploadChangeParam<any>) => {
  baseFormApi.getValues().then((res) => {
    importProductOrg({ file: info.file, ...res, ...extParams.value }).then(
      (res) => {
        if (res.success) {
...
<template>
  <Modal class="w-[500px]" title="导入" title-tooltip="上传excel文件导入数据">
    ...
    <div class="pl-[10px] pr-[10px]">
      ...
      >
        <AntdDownloadOutlined class="mr-[6px] text-[14px]" />采购协议模版下载
      </a>
      <UploadDragger
