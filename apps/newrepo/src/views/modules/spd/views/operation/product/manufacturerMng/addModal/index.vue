...
import { saveManufacturer } from '../api';
...
const manufacturerId = ref<null | number | string>(null);
...
const [ModalFirst, modalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    try {
      ...
      const response = await saveManufacturer(params);
      if (response.success) {
        modalApi.close();
  ...
  onOpenChange(isOpen: boolean) {
    if (isOpen) {
      ...
      if (data.value.openType === 'add') {
        ...
        editFormApi.setValues(formData);
        title.value = '编辑';
        manufacturerId.value = data.value.formData?.manufacturerId || null;
        showForm.value = data.value.formData?.showForm;
        showFormLast.value = data.value.formData?.showFormLast;
      }
    } else {
