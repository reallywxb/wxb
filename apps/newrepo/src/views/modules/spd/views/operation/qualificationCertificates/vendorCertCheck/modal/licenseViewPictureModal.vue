...
const handlePass = () => {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        const data = state.value?.data;
        const processedParams = data.processedData; // 获取预处理的参数
        const params = {
          checkRemark: '',
          type: processedParams.type, // 使用预处理的type
          status: processedParams.isWA ? 'WC' : 'PS', // 根据预审状态决定目标状态
          ids: JSON.stringify([
            {
              applySyncId: processedParams.applySyncId,
              applyCertId: processedParams.applyCertId,
            },
          ]),
        };
        console.warn('通过操作参数:', params);
        ...
        if (result && result.success) {
          ...
          if (data.updateStatusCallback) {
            const targetStatus = processedParams.isWA ? 'WA' : 'PS';
            data.updateStatusCallback({
              status: targetStatus,
              statusText: processedParams.isWA ? '已预审' : '已核对',
              statusColor: '#009688',
            });
          }
          modalApi.close();
...
// 拒绝
const handleReject = () => {
  let rejectReason = state.value?.data?.checkRemark || '';
  Modal.confirm({
    title: '请输入驳回原因',
    content: h('textarea', {
      placeholder: '',
      ...
      'data-testid': 'textarea_reject_reason_licenseViewPictureModal',
    }),
    width: '400px',
    centered: true,
    async onOk() {
      ...
      try {
        const data = state.value?.data;
        const processedParams = data.processedData;
        // 构造请求参数
        const params = {
  ...
  <ModalFirst title="证照图片查看" class="h-[650px] w-[1300px]">
    <div class="flex h-full gap-3">
      <div class="fieldset-box w-8/12" title="证照图片">
        <div class="flex h-full gap-2 px-2.5 py-3">
          <div class="flex h-20 w-full border border-[#b1b1b1] py-2">
            ...
            <div
              ...
              :data-testid="`button_image_${imageIndex}_licenseViewPictureModal`"
            >
              <div
    ...
    <template #prepend-footer>
      <Button
        type="primary"
        @click="handlePass"
        data-testid="button_pass_licenseViewPictureModal"
      >
        通过
        <template #icon>
      ...
      <Button
        ...
        danger
        @click="handleReject"
        data-testid="button_reject_licenseViewPictureModal"
      >
        拒绝
        <template #icon>
