...
const C_UOM_ID = ref<null | number | string>(null);
...
const [ModalFirst, modalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    try {
      ...
      const params = {
        ...values,
        C_UOM_ID: C_UOM_ID.value,
      };
      const response = await saveUom(params);
      if (response.success) {
        modalApi.close();
  ...
  onOpenChange(isOpen: boolean) {
    if (isOpen) {
      ...
      if (data.value.openType === 'add') {
        ...
        title.value = '编辑';
        C_UOM_ID.value = data.value.formData?.C_UOM_ID || null;
        showForm.value = data.value.formData?.showForm;
        showFormLast.value = data.value.formData?.showFormLast;
      }
    } else {
