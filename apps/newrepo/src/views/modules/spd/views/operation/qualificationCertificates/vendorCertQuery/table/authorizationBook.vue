...
const [ChcGrid, ChcGridApi] = useSpdGrid(
  ...
  },
  {
    ...
    dataTableId: 'authorizeAction/queryAuthorize.do',
...
watch(
  ...
  (val: string, oldVal: string) => {
    console.warn('currentTab-authorizationBook===>', val, oldVal);
    if (val === props.thisTab.value && selectedParentRow?.value?.bpartnerId) {
      console.warn('inject-authorizationBook===>', selectedParentRow);
      const params = {
        vendorId: selectedParentRow?.value?.bpartnerId,
        bpartnerId: selectedParentRow?.value?.bpartnerId,
        validation: `ac.c_bpartner_id=${selectedParentRow?.value?.bpartnerId}`,
      };
      ChcGridApi.reload({
        ...params,
      });
...
defineExpose({
  query() {
    // 子表请求
    console.warn('authorizationBook子表请求被触发===>');
    ChcGridApi.reload({
      vendorId: selectedParentRow?.value?.bpartnerId,
      bpartnerId: selectedParentRow?.value?.bpartnerId,
      validation: `ac.c_bpartner_id=${selectedParentRow?.value?.bpartnerId}`,
    ...
    <ChcGrid>
      <template #toolbar-actions>
        <Form
          ...
          layout="inline"
        >
          <FormItem label="生产企业" name="manufacturerName">
            <Input
          ...
          >
            <Select
              ...
              data-testid="select_certvalid_authorizationBook"
            />
          </FormItem>
          <FormItem>
            <Button
              ...
              html-type="submit"
              class="mr-[0.5rem]"
              data-testid="button_query_authorizationBook"
            >
              查询
              <template #icon>
                <SearchActionIcon />
      ...
      <template #action="scope">
        <Button
          ...
          :data-testid="`button_viewLicense_${scope.rowIndex}_authorizationBook`"
        >
          授权书详情
          <template #icon>
            <IconfontBasicView />
