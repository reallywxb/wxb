...
import excelUrl from '#/assets/excels/user.xls?url';
...
const handleChange = (info: UploadChangeParam<any>) => {
  baseFormApi.getValues().then((res) => {
    ...
    // // 关闭模态框
    // modalApi.close();
    importUser({ file: info.file }).then((res) => {
      if (res.success) {
  ...
  >
    ...
    <div class="pl-[10px] pr-[10px]">
      <a
        ...
        data-testid="button_download"
      >
        <AntdDownloadOutlined class="mr-[6px] text-[14px]" />用户信息下载
      </a>
      <UploadDragger
        ...
        class="mt-[16px]"
        data-testid="button_upload_dragger"
      >
