...
export function getValidateRule(item: any) {
  ...
  const checkConfig = (rule: any, value: any, callback: any) => {
    ...
    if (itemProps.min) {
      const minDate = util.momentFunc(itemProps.min, 'YYYY-MM-DD');
      const valueDate = util.momentFunc(value, 'YYYY-MM-DD');
    ...
    if (itemProps.max) {
      const maxDate = util.momentFunc(itemProps.max, 'YYYY-MM-DD');
      const valueDate = util.momentFunc(value, 'YYYY-MM-DD');
