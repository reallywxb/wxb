...
<template>
  <ul
    :class="[
      ...
      is(theme, true),
      is('rounded', rounded),
      place === 'sidebar' ? 'isInSidebar' : 'isInHeader',
    ]"
    class="relative"
  >
...
.#{$namespace}-normal-menu {
  ...
  &__item {
    @apply dark:text-foreground/90 text-sidebar-foreground;
    ...
    &.is-active {
      @apply dark:text-primary dark:bg-accent bg-sidebar-accent-active;

      .#{$namespace}-normal-menu__name,
      .#{$namespace}-normal-menu__icon {
        @apply dark:text-primary-foreground text-sidebar-accent-foreground font-semibold;
      }
    }

    &:not(.is-active):hover {
      @apply dark:bg-accent dark:text-foreground bg-sidebar-accent-hover text-sidebar-foreground;
    }
