...
export class FormApi {
  ...
  getAdvancedSearchValues: () => { conditions: Array<any> } = () => {
    return {
      conditions: [],
    };
  };
  ...
  async getValues<T = Recordable<any>>() {
    ...
    const conditions = this.getAdvancedSearchValues().conditions; 
    return (
      form.values
        ? {
            ...this.handleRangeTimeValue(form.values),
            ...(conditions.length > 0
  ...
  updateAdvancedSearchValuesFn(fn: () => { conditions: Array<any> }) {
    this.getAdvancedSearchValues = fn;
  }
  ...
  private handleRangeTimeValue = (originValues: Record<string, any>) => {
    ...
    fieldMappingTime.forEach(
      ([field, [startTimeKey, endTimeKey], format = 'YYYY-MM-DD']) => {
        ...
        // 先删除再赋值，避免startTime字段名与form字段名相同时startTime值被删除
        Reflect.deleteProperty(values, field);
