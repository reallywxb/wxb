...
export type AdvSearchCompMap =
  ...
  | 'TimeGroup'
  | 'TimePicker';
export type OperatorsTypeMap = 'number' | 'select' | 'text' | 'time';
export type AdvSearchItem = {
  /**
   * 使用的组件
   */
  component: AdvSearchCompMap;
  /**
   * 组件的配置对象
   */
   ...
   * 运算符种类
   */
  operatorOptions?: OperatorsTypeMap;
  /**
   * 查询项的值
   */
  value?: any;
...
> extends Omit<
   ...
   * 高级查询相关配置
   */
  queryAdvSearchKeys?: () => Promise<AdvSearchField[]>;
  /**
   * 范围查询字段参数起始值后缀
   */
  rangeFieldFromSuffix?: string;

  /**
   * 范围查询字段参数结束值后缀
   */
  rangeFieldToSuffix?: string;
