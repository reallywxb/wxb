...
import type { UseVbenVxeGrid, VbenVxeGridProps } from '@vben/plugins/vxe-table';
...
export const useEditableApi = <T extends string>({
  ...
  beforeDropdownVisible?: (open: boolean) => Promise<void> | void;
  ...
  const isResetting = ref(false); // 重置标记
              ...
              if (insertRows.length === 0) {
                ...
                if (scope.$grid.isInsertByRow(scope.row)) {
                  ...
                  if (isOnEnterSave.value) {
                    ...
                    await handleInsertRowsPromise(scope.row);
                  }
                  // await handleInsertRowsPromise(scope.row);
                  // const insertRow = await handleInsertRowsPromise(scope.row);
                  // scope.$grid.setEditRow(insertRow, true);
  ...
  function handleInsertRowsPromise(row: any) {
    return new Promise((resolve) => {
      ...
      setTimeout(async () => {
        ...
        midRows.forEach(async (item: any, i: number) => {
          await chcGridApi.value.grid.insertAt(item, -1).then((midRow: any) => {
            if (i === 0) {
              newRow = midRow.row;
              resolve(newRow);
            }
          ...
          function handleInsertRows() {
            ...
            return new Promise((resolve) => {
              ...
              setTimeout(async () => {
                ...
                // }
                midRows.length > 0 &&
                  chcGridApi.value.grid.setEditRow(midRows[0], true);
                resolve(true);
              }, 200);
            });
          }
  ...
  const handleDropdownVisibleChange = async (open: boolean) => {
    ...
    await beforeDropdownVisible?.(open);
    // 打开时重置查询关键字并刷新数据
    if (open && chcSelectRef.value) {
      isResetting.value = true;
      try {
        if (chcSelectRef.value.params) {
          // chcSelectRef.value.params.productName = '';
          const p = chcSelectRef.value.params ?? {};
          ['productName', 'query', 'value'].forEach((k) => {
            if (k in p) p[k] = '';
          });
          chcSelectRef.value.params = p;
        }
        if (typeof chcSelectRef.value.fetchApi === 'function') {
          await chcSelectRef.value.fetchApi();
        }
      } finally {
  ...
  return {
    ...
    handleSelectParams, // 处理下拉组件查询参数
    getVxeNumIptProps, // 获取数字输入框属性
  };
};
export const createEditableGrid = (
  useVbenVxeGrid: UseVbenVxeGrid,
  namespace: string,
) => {
  return (params: VbenVxeGridProps) => {
    if (!params.gridOptions?.id) {
      console.error('缺少gridOptions.id参数,请在配置项里添加');
    }
          ...
          restoreStore: () => {
            return new Promise((resolve) => {
              queryDataTableColumnConfig(
                ...
                params.gridOptions?.id
                  ? `editTable_${params.gridOptions?.id as string}`
                  : `editTable_${namespace}${location.pathname}`, // schemaColumnAndOptions.customColumnsConfigKey
              ).then((res) => {
                if (res && res.data) {
          ...
          updateStore: (param: any) => {
            return new Promise((resolve) => {
              saveDataTableColumnConfig(
                ...
                params.gridOptions?.id
                  ? `editTable_${params.gridOptions?.id as string}`
                  : `editTable_${namespace}${location.pathname}`,
                param.storeData,
              ).then(() => {
                resolve(null);
              });
