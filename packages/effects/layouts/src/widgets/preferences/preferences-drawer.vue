...
async function handleReset() {
  ...
  await handleReset(); // 此处需要执行两次，才能让顶栏和侧边栏回复初始状态，因为第一次时会触发顶栏边栏自动和主题色一致，然后再执行一次，才能让顶栏边栏恢复
}
</script>
          ...
          <template #appearance>
            ...
            <Block :title="$t('preferences.theme.builtin.title')">
              <BuiltinTheme
                ...
                :is-dark="isDark"
                v-model:theme-header-color="themeHeaderColor"
                v-model:theme-sidebar-color="themeSidebarColor"
              />
            </Block>
            <Block :title="$t('preferences.theme.headerAndSidebar.title')">
              <HeaderAndSidebarColors
                v-model:theme-header-color="themeHeaderColor"
                v-model:theme-sidebar-color="themeSidebarColor"
              />
