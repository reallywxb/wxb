...
import { cloneDeep, cn, isBoolean, mergeWithArrayOverride } from '@vben/utils';
...
// let originOptions: VxeTableGridProps<any>;
const options = computed(() => {
  ...
  // if (originOptions) {
  //   return Object.assign(originOptions, mergedOptions);
  // } else {
  //   originOptions = mergedOptions;
...
async function init() {
  await nextTick();
  const globalGridConfig = VxeUI?.getConfig()?.grid ?? {};
  globalGridConfig.proxyConfig &&
    (globalGridConfig.proxyConfig.autoLoad = false);
    ...
    >
      ...
      <template #form>
        <div
          ...
          :class="cn('relative rounded py-1', isCompactForm ? 'pb-4' : 'pb-4')"
        >
          <slot name="form">
            <Form>
              <template
