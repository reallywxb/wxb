...
export function useAntdDesignTokens() {
  ...
  const tokens = reactive({
    ...
    // colorTextDisabled: 'rgba(0, 0, 0, 0.6)', // 组件disabled时的文本颜色
    colorTextDisabled: 'hsl(var(--foreground) / 50%)', // 组件disabled时的文本颜色
    // colorBgTextHover: 'rgba(0, 0, 0, 0.06)',hsl(var(--primary) / 25%)
    controlItemBgActive: 'hsl(var(--primary) / 16%)',
    controlItemBgHover: 'hsl(var(--primary) / 8%)', // select的item项的鼠标悬停背景色
